...
int
buf_nextrune(Buffer *buf, char **p, Rune *r)
{
	int runelen;
	...
	runelen = chartorune(r, *p);
	(*p)++;
	return runelen;
}

int
buf_insertafter(Buffer *buf, char *s, int n)
{
	int i;
...
int
buf_insertbefore(Buffer *buf, char *s, int n)
{
	int i;
